...
{    
    ...
    TWLWriteByteReg(hTwl, TWL_MEMORY_ADDRESS, (0x2B << 2));
    for (i = 0; i < T2_SLEEPOFF_COUNT; ++i)
        TWLWriteByteReg(hTwl, TWL_MEMORY_DATA, s_pT2SleepOffValue[i]);

    // program sleep to active sequence (address autoincrements)
    TWLWriteByteReg(hTwl, TWL_MEMORY_ADDRESS, (0x2D << 2));
    for (i = 0; i < T2_WAKEON_COUNT; ++i)
        TWLWriteByteReg(hTwl, TWL_MEMORY_DATA, s_pT2WakeOnValue[i]);
    ...
    TWLWriteByteReg(hTwl, TWL_MEMORY_ADDRESS, (T2_A2S_SEQ_START_ADDR << 2) );
    ...
    TWLWriteByteReg(hTwl, TWL_MEMORY_ADDRESS, (T2_S2A_SEQ_START_ADDR << 2) );
    ...
    TWLWriteByteReg(hTwl, TWL_MEMORY_ADDRESS, (T2_WARMRESET_SEQ_START_ADDR << 2));
    ...
    TWLWriteByteReg(hTwl, TWL_P1_SW_EVENTS, 0x18);
