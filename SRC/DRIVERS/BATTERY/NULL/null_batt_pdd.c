...
#define USERACTIVITYTIMER_RETSET_EVENT_NAME  	_T("PowerManager/ActivityTimer/UserActivity")

static HANDLE h_UserActiveEvent = NULL;
    ...
    )
{
	...
	h_UserActiveEvent = OpenEvent(EVENT_ALL_ACCESS, FALSE, USERACTIVITYTIMER_RETSET_EVENT_NAME);
	if (h_UserActiveEvent == NULL)
	{
		RETAILMSG(1, (TEXT("Batt_pdd: Open user active event failed\r\n")));        
	}
    DEBUGMSG(ZONE_FUNCTION, (L"-BatteryPDDInitialize\r\n"));
    return TRUE;
}
    ...
    ) 
{
	...
	if (h_UserActiveEvent != NULL)
	{
		// Do set event more than one times ?
		SetEvent(h_UserActiveEvent);
	}
