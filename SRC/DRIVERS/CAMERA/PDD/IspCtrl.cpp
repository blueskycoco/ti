...
BOOL CIspCtrl::ConfigGPIO4MDC()
{
    ...
    // set GPIO58 0 -> 1 (reset)
		...
		//gpio pinmuxed for mmc2 ,so del it
		/*
		GPIOinBuffer[0] = 134;
		GPIOinBuffer[1] = GPIO_DIR_OUTPUT;
		if (DeviceIoControl(hGPIO, IOCTL_GPIO_SETMODE, &GPIOinBuffer, 
		...
		*/
		...
		{
		    DEBUGMSG(ZONE_ERROR, (TEXT("Port Closeing isn't successed. \r\n")));    
		    return FALSE;
		}		
#endif
...
{    
    ...
    if(!CCDCInit(pCsDataRangeVideo)) // Init. CCDC               
        return FALSE;
ISPConfigSize(0, 0, 0);// Init. video size 
        
		memset(m_pYUVVirtualAddr,0,IMAGE_CAMBUFF_SIZE); //clear the frame buffer
