...
#define I2C2_MEMBASE 					0x48072000
//#ifdef BSP_EVM2
...
BOOL CTvpCtrl::WriteReg(UINT8 slaveaddress,UINT8 value)
{
    BOOL rc = FALSE;
	//unsigned int dwTemp;
	//volatile unsigned int *pPadCfg = (volatile unsigned int *)0xB60021C0;
	//dwTemp = *pPadCfg;
	//RETAILMSG(1, (L"Cam:PadCfg:0x%08x\r\n", dwTemp));
    if (m_hI2C)
    {
...
{
    m_hI2C = I2COpen(OMAP_DEVICE_I2C2); //I2CGetDeviceIdFromMembase(I2C2_MEMBASE)
    if (m_hI2C == NULL)
        return FALSE;
