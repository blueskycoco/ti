...
/*		define mac address	*/
#define MAC21	0x1100//0xa02f//
#define MAC43	0x3322//0xfef9//
#define MAC65	0xa640//0x5544//

DWORD g_dwDm9Load=0;

void RegisterMacAddress();
//extern U16 MAC_Random[3];
U8 MAC_Random[6] = {0x00,0x11,0x22,0x33,0,0};
void Gen_Mac_Random()
{
	int i;
	for(i = 4; i < 6; i ++)
		MAC_Random[i] = (U8)(Random() % 0xff);
}
	...
	PU8		ptrBuffer)
{
	...
	RegisterMacAddress();
	for(i = 0; i < 6; i ++)
		RETAILMSG(0,(TEXT("Macaddress write to DM9000PAR %d= 0x%x \r\n"),i,MAC_Random[i]));
	*(PU16)ptrBuffer =( (MAC_Random[1]  & 0xff)<<8) | (MAC_Random[0] & 0xff);
	*(PU16)(ptrBuffer+2) = ( (MAC_Random[3]  & 0xff)<<8)  | (MAC_Random[2] & 0xff);
	*(PU16)(ptrBuffer+4) =  ( (MAC_Random[5]  & 0xff)<<8)  | (MAC_Random[4] & 0xff);
...
void RegisterMacAddress( )
{
	...
	long lStatus;
	int i;
	//LPCWSTR szRegPath = TEXT("Comm\\DM9CE1\\Parms");
	static const TCHAR szRegPath[]		= TEXT("Comm\\DM9CE1\\Parms");
	BYTE Mac_From_Reg[128];
	BYTE Mac_Writeto_Reg[6];
	DWORD dwType=REG_BINARY;
	DWORD dwValLen = sizeof(Mac_From_Reg);
	RETAILMSG(0,(TEXT("RegisterMacAddress++++\r\n")));
	lStatus = RegOpenKeyEx(HKEY_LOCAL_MACHINE, szRegPath, 0,0, &hKey);
    
	if (lStatus==ERROR_SUCCESS) 
		{
			RETAILMSG(0,(TEXT("RegisterMacAddress1++++\r\n")));
		       lStatus = RegQueryValueEx(
		        ...
		        if (lStatus == ERROR_SUCCESS) 
					{
						RETAILMSG(0,(TEXT("RegisterMacAddress2++++\r\n")));
						for(i = 0; i < 6; i ++)
							RETAILMSG(0,(TEXT("Macaddress in Regedit %d= 0x%x \r\n"),i,Mac_From_Reg[i]));
						dwValLen = 6;
						if (!(Mac_From_Reg[0] | Mac_From_Reg[1]|Mac_From_Reg[2] | Mac_From_Reg[3]|Mac_From_Reg[4] | Mac_From_Reg[5]))
							{
								Gen_Mac_Random();
								for(i = 0; i < 6; i++)
									{
										Mac_Writeto_Reg[i] = MAC_Random[i];
								}
...
*/
...

	{
		...
		if(!(uBase = (U32)MmMapIoSpace(phyAddr, 4, FALSE)))
			THROW((ERR_STRING("Fails to map io space")));

		DEBUG_PRINT((
			TEXT("[dm9: The IO mapped address is %X\r\n"), uBase
			));	

		m_szCurrentSettings[SID_ADDR_BASE_ADDRESS] = uBase;	
	}

	//RETAILMSG (1,(TEXT("[DM9000A] m_szCurrentSettings[SID_ADDR_BASE_ADDRESS] = 0x%x \n\r"), m_szCurrentSettings[SID_ADDR_BASE_ADDRESS]));

	{
		phyAddr.HighPart = 0;
		phyAddr.LowPart = m_szConfigures[CID_DATA_BASE_ADRESS];
	
		DEBUG_PRINT((
			TEXT("[dm9: Tries to map Data space with %X\r\n"), phyAddr.LowPart
			));
		...
		DEBUG_PRINT((
			TEXT("[dm9: The Data mapped address is %X\r\n"), uBase
			));	

		m_szCurrentSettings[SID_DATA_BASE_ADDRESS] = uBase;	
	}

	//RETAILMSG (1,(TEXT("[DM9000A] m_szCurrentSettings[SID_DATA_BASE_ADDRESS] = 0x%x \n\r"), m_szCurrentSettings[SID_DATA_BASE_ADDRESS]));
#endif
}
...
void	NIC_DEVICE_OBJECT::EDeviceRegisterInterrupt(void)
{
	if(!m_szConfigures[CID_NEED_INTERRUPT]) return;
		...
		DEBUG_PRINT((
			TEXT("[dm9:CID_IRQ_NUMBER %x CID_IRQ_SHARED%X  %XCID_IRQ_GEN_TYPE \r\n"), m_szConfigures[CID_IRQ_NUMBER],m_szConfigures[CID_IRQ_SHARED]
			,m_szConfigures[CID_IRQ_GEN_TYPE]
			));
		
	NDIS_STATUS	status;

	if(g_dwDm9Load>0)
		THROW((ERR_STRING("Error in registering interrupt"),0xc0000001));
